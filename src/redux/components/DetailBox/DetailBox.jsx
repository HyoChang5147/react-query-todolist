...
import { KEY_QUERIES } from "../../../query/keys.constant";
...
function DetailBox() {
  ...
  const {
    ...
    isError,
    data: todos,
  } = useQuery(KEY_QUERIES.TODOS, getTodos, {
    retry: 3,
  });

  // 이 컴포넌트에서 아이템을 사용하기 위해, params로 전달받은 id를 이용-todo를 filtering

  const filteredTodos = todos?.filter((item) => item.id === params.id);
